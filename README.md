...
## API documentation
Swagger documentation is available in **api-docs.json** file

## Tests
Tests could be executed by
...
### Run on a local PC
To run application on a local PC use
...
To run application in a Docker container use

```
./mvnw package
docker build -t "ubitricity_code_challenge" .
docker run -d -p 8080:8080 ubitricity_code_challenge
```
